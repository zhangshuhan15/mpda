...
import com.dahuaboke.mpda.rag.reranker.CustomRerankInput;
import com.dahuaboke.mpda.rag.reranker.CustomRerankOutPut;
...
public class RerankerClientManager {
    ...
    public RerankResponse call(RerankRequest request) {
        ...
        CustomRerankInput rerankInput = this.createRequest(request);
        ResponseEntity<List<CustomRerankOutPut>> customRerankOutPut = this.retryTemplate.execute((ctx) -> rerankEntity(rerankInput));
        List<CustomRerankOutPut> response = customRerankOutPut.getBody();
        if (response == null) {
    ...
    private CustomRerankInput createRequest(RerankRequest request) {
        List<String> docs = request.getInstructions().stream().map(Document::getText).toList();
        return new CustomRerankInput(model, request.getQuery(), docs);
    }

    private ResponseEntity<List<CustomRerankOutPut>> rerankEntity(CustomRerankInput rerankInput) {
        Assert.notNull(rerankInput, "The request body can not be null.");
        ResponseEntity<List<CustomRerankOutPut>> entity = this.restClient.post().uri(rerankPath)
