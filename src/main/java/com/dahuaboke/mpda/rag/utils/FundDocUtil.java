package com.dahuaboke.mpda.rag.utils;

import com.dahuaboke.mpda.client.RerankerClientManager;
import com.dahuaboke.mpda.rag.advisor.CustomQuestionAnswerAdvisor;
import com.dahuaboke.mpda.rag.config.CustomKeywordEnricher;
import com.dahuaboke.mpda.rag.config.CustomSummaryEnricher;
import com.dahuaboke.mpda.rag.entity.FundFieldMapper;
import com.dahuaboke.mpda.rag.entity.FundProduct;
import com.dahuaboke.mpda.tools.product.ProductToolHandler;
import com.dahuaboke.mpda.tools.product.entity.YwjqrProduct;
import com.dahuaboke.mpda.utils.SpringUtil;
...
public class FundDocUtil {
    ...
    private static final RerankerClientManager rerankerClientManager;
    ...
    static {
        ...
        rerankerClientManager = SpringUtil.getBean(RerankerClientManager.class);
    }
        ...
        for (int i = 0; i < resources.length; i++) {
            ...
            try {
                ...
                try (InputStream is = resource.getInputStream()) {
                    ...
                    CustomKeywordEnricher customKeywordEnricher = new CustomKeywordEnricher(chatModel);
                    customKeywordEnricher.setPrompt(getPrompt());
                    List<Document> keywordDocs = customKeywordEnricher.apply(docs);


                    CustomSummaryEnricher summaryMetadataEnricher = new CustomSummaryEnricher(chatModel,
                            List.of(CustomSummaryEnricher.SummaryType.PREVIOUS,
                                    CustomSummaryEnricher.SummaryType.CURRENT, CustomSummaryEnricher.SummaryType.NEXT)
                    );
                ...
                for (Map.Entry<String, String> entry : entries) {
                    ...
                    try {
                        String content = chatClient.prompt()
                                .advisors(CustomQuestionAnswerAdvisor
                                        .builder(vectorStore, rerankerClientManager)
                                        .keys(List.of(key))
                                        .productName(List.of(productName))
                                        .searchRequest(MilvusSearchRequest.milvusBuilder()
                                                .topK(15)
                                                .query(userQuery)
